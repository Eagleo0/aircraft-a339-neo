...
exports.getGitBuildInfo = () => {
  ...
  try {
    const commitHash = process.env.GITHUB_SHA ? process.env.GITHUB_SHA : evaluate('git rev-parse HEAD');
    let tag = '';
    try {
    ...
    const buildInfo = {
      ...
      ref: process.env.GITHUB_REF ? process.env.GITHUB_REF : evaluate('git symbolic-ref HEAD').replace(/.+\//, ''),
      commitHash,
